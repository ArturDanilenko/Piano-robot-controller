...
void loop() {
   ...
   currentAngle = posToAngle(position);
  ...
  ======================================================================*/
   ...
   // take in desired X,y into xy_des array, x[0], y[1]

  // angle_in = invKin(xy_des);                //read from rotary encoder connected to A0
  angle_in[0] = invKin0(xy_des[0],xy_des[1]);
  angle_in[1] = invKin1(xy_des[0],xy_des[1]);
  angle_in_r = angle_in[0];
  angle_in_l = angle_in[1];              //read from rotary encoder connected to A0
  ...
  //if angle out_l < 0 left motor backwards (fliped cus gears)
  if (angle_out_l < 0){
  // L_Motor_F_state = LOW;
  // L_Motor_R_state = HIGH;
  }
  else if (angle_out_l > 0){  // else if angle_out_l > 0 left motor forwards
  //  L_Motor_F_state = HIGH;
    // L_Motor_R_state = LOW;
  }
  else{     //else motor stop
    // L_Motor_F_state = LOW;
    // L_Motor_R_state = LOW;
  }
